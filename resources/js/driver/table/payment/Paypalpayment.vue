<template>
    <nav aria-label="breadcrumb" class="mb-2">
      <ol class="breadcrumb breadcrumb-pipes">
        ...
        <li class="breadcrumb-item active" aria-current="page">{{ $t('Paypal Payments') }}</li>
      </ol>
    </nav>
      ...
      <div class="col-12">
        ...
        <div class="card-header">
          ...
          <div class="card-header d-flex justify-content-between align-items-center">
            <div>
              <h4 class="card-title">{{ $t('Paypal Payments') }}</h4>
  ...
  export default {
    ...
    goToInvoice() {
      if (this.invoiceId) {
        ...
        window.open(`/payment_paypal/success/${this.invoiceId}`, '_blank');
      } else {
      ...
      updateStatus() {
        axios
          .post(`/api/paypal/status/${this.selectedPaymentId}`, { status: this.selectedStatus }, {
            headers: {
