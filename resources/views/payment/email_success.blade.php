...
<body>
	<div class="container-btn">
		...
		<button class="print-button" onclick="window.print()"
			style="margin-bottom: 20px; padding: 10px 15px; background: #007bff; color: white; border: none; cursor: pointer; border-radius: 5px;">
			Print Invoice
		</button>

		{{-- button go home --}}
		<a href="{{ route('home') }}" onclick="clearCartAndRedirect(event)"
			style="margin-bottom: 20px; padding: 10px 15px; background: #d9b382; color: white; border: none; cursor: pointer; border-radius: 5px; text-decoration: none;">
			Go to Home
		</a>
	</div>
	<div class="container">
		...
		<div class="header">
			...
			@endif
			{{-- <p>Zahlungsart: {{ $payments[0]->payment_method === 'paypal' ? 'Paypal' : 'Auf Rechnung' }}
			</p> --}}
		</div>
		...
		{{-- <div class="totals">
            <p>19% USt: ${{ $total_vat }} + ${{ $total_amount }}= ${{ $total_total }}</p>
		<p><strong>Noch zu zahlender Betrag:
				{{ $payments[0]->payment_method === 'paypal' ? '$0.00' : '$' . $total_total }}</strong>
		</p>
	</div> --}}

	<p class="important">Bitte Überweisen Sie den Beitrag innerhalb von 14 Tagen an die unten dargestellten Bank
		Verbindung</p>
	<p class="important">
